...
### 3.3.1 CSP とリッチエディタの現実的な妥協案

Tiptap 等のリッチテキストエディタは動的に `style="..."` を生成することがあり、厳格に `unsafe-inline` を禁止した CSP と競合する場合があります。

- サーバー側で出力を正規化/サニタイズ（推奨）:
    - サーバ保存時にエディタの HTML/ブロック表現をサニタイズし、`style` 属性は許容されるプロパティと値だけにフィルタするか、可能であれば CSS クラスに変換する。これによりグローバルな CSP を厳格に保てます。
    - 長所: 他画面への影響がなくセキュリティが高い。短所: 実装コストがある（スタイルのマッピングが必要）。

`dangerouslySetInnerHTML` の使用は避け、出力 HTML のサニタイズ（`rehype-sanitize` 等）を常に行ってください。
